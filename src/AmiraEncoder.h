...
//For the half-step state table (emits a code at 00 and 11).
...
//For the full-step state table (emits a code at 00 only).
...
class Encoder {
  public:
    Encoder(uint8_t _pinA, uint8_t _pinB, uint8_t _pullup, uint8_t _stepMode, uint8_t _encSens);
  ...
  private:
    ...
    uint8_t pinB;
    uint8_t pullup;
    uint8_t stepMode;                 //Defines if you use half step or full step mode table.
